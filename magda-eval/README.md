...
### report_final 分层汇总（报告写作）

对 `eval_report/report_final/*.json` 批量生成分层产物与跨运行对比表：

```bash
node magda-eval/scripts/summarize-report-final.mjs
```

输出见 **`eval_report/report_final/README.md`**（`comparison-by-run.csv`、`comparison-by-family.csv`、`comparison-case-matrix.csv`、每报告 `*-hierarchy-summary.json` 等）。

单报告：

```bash
node magda-eval/scripts/extract-report-hierarchy.mjs <report.json> --out eval_report/report_final --csv
```
...
4. 选择 slug、确认数据集已从 registry 加载后，点击 **「运行评测」**。**Eval pipeline**：**Agent — deterministic**（默认，task-spec + 确定性 SQL）；**Agent — planner only**（消融，每题 Planner LLM）；**Baseline direct**（无 AgentChain，profile + 问题 → 单次 LLM SQL）。报告 `meta.eval_pipeline` 为 `agent`、`agent_full_planner` 或 `baseline_direct`。
